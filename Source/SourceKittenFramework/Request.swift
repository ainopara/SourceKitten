...
public class RequestBuilder {
    ...
    private func transform(any: Any) -> sourcekitd_object_t {
        ...
        default:
            fatalError("RequestBuilder: Unsupported value type: \(type(of: any))")
        }
    }
}
...
public struct Request {
    ...
    public struct EditorOpen: RequestType {
        ...
        public func sourcekitObject() -> sourcekitd_object_t {
            ...
            switch source {
            case .path(let path):
                requestBuilder[.name] = path
                requestBuilder[.sourceFile] = path
            case .text(let text):
                requestBuilder[.name] = String(text.hash)
                requestBuilder[.sourceText] = text
            }
    ...
    public static func editorOpen(file: File, arguments: [String] = defaultArguments) -> Request {
        ...
        return .editorOpen(source: source, arguments: arguments)
    }
    ...
    public struct EditorOpenInterface: RequestType {
        ...
        public enum Group {
            case name(String)
            case containsInterestedUSR(String)
        }
        ...
        public func sourcekitObject() -> sourcekitd_object_t {
            ...
            if let group = self.group {
                switch group {
                case .name(let name):
                    requestBuilder[.groupName] = name
                case .containsInterestedUSR(let interestedUSR):
                    requestBuilder[.interestedUSR] = interestedUSR
                }
            }
    ...
    public struct CodeComplete: RequestType {
        public struct Source {
            ...
            public static func text(_ text: String, id: String? = nil) -> Source {
                return Source(sourceFile: id, sourceText: text)
            }
        }
        public let source: Source
        public let offset: Int64
    ...
    public static func codeCompletionRequest(file: String, contents: String, offset: Int64, arguments: [String]) -> Request {
        return .codeComplete(source: .text(contents, id: file), offset: offset, arguments: arguments)
    }
