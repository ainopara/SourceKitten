...
public enum CursorInfoLocation {
    case offset(Int64)
    case usr(String)
}
...
public enum Request {
    ...
    fileprivate var sourcekitObject: sourcekitd_object_t {
        ...
        case .cursorInfo(let file, let location, let arguments):
        ...
        case .moduleGroups(let module, let arguments):
            var compilerargs = arguments.map({ sourcekitd_request_string_create($0) })
            dict = [
    ...
    */
    internal static func cursorInfoRequest(filePath: String?, arguments: [String]) -> sourcekitd_object_t? {
        if let path = filePath {
            return Request.cursorInfo(file: path, location: .offset(0), arguments: arguments).sourcekitObject
        }
        return nil
    }
